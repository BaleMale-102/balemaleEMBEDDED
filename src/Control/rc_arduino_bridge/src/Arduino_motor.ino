...
static void stopAll(){
  MOTORA_STOP(0); MOTORB_STOP(0); MOTORC_STOP(0); MOTORD_STOP(0);
}
...
static void setMotorSigned(char m, int s_pwm){
  ...
  if (m=='A') forward ? MOTORA_FORWARD(pwm) : MOTORA_BACKOFF(pwm);
  if (m=='B') forward ? MOTORB_FORWARD(pwm) : MOTORB_BACKOFF(pwm);
  if (m=='C') forward ? MOTORC_FORWARD(pwm) : MOTORC_BACKOFF(pwm);
  if (m=='D') forward ? MOTORD_FORWARD(pwm) : MOTORD_BACKOFF(pwm);
}
...
static bool readLine(String &out){
  while (SERIAL.available()){
    ...
    if (c != '\r') rx_buf += c;
    if (rx_buf.length() > 100) rx_buf = ""; // runaway protection
  }
  return false;
}
...
static void handlePCommand(long v){
  ...
  if (v < MIN_PWM + 50) v = MIN_PWM + 50; // MAX는 MIN보다 커야 함
  if (v > 4000) v = 4000;
  MAX_PWM = (int)v;

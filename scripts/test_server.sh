...
# 경로는 전체 왕복 경로 (origin → parking area → home)
...
show_help() {
    ...
    echo "  verify <슬롯> [전체경로]      서버 응답 시뮬레이션"
    echo "  full <번호판> <슬롯> [전체경로]  입고 전체 플로우 시뮬레이션"
    echo "  exit <전체경로> <슬롯>        출차 테스트 (슬롯에서 차량 회수)"
    echo "  status                        현재 상태 확인"
    echo ""
    echo "  전체경로 = 서버 왕복 경로 (origin → parking area → home)"
    echo ""
    echo "Examples:"
    echo "  $0 wait                       # 대기 상태 진입"
    echo "  $0 plate 12가3456             # 번호판 인식"
    echo "  $0 verify 17 0,1,5,1,0        # 서버 응답 (슬롯17, 왕복경로)"
    echo "  $0 full 12가3456 17 0,1,5,1,0 # 입고 전체 테스트"
    echo "  $0 exit 0,1,5,1,0 17          # 출차 테스트 (왕복경로→슬롯17)"
    ...
    echo "3. 서버 응답 (전체 왕복 경로 포함) 받으면 자동으로 주행 시작"
    echo ""
...
case "$1" in
    ...
    verify)
        if [ -z "$2" ]; then
            echo "Usage: $0 verify <슬롯> [전체경로]"
            echo "Example: $0 verify 17 0,1,5,1,0"
            exit 1
        fi
        SLOT=$2
        PATH_STR=${3:-""}
        if [ -n "$PATH_STR" ]; then
            echo "Simulating server response: slot=$SLOT, full_path=$PATH_STR"
            ros2 topic pub --once /mission/test_cmd std_msgs/String "data: 'VERIFY $SLOT $PATH_STR'"
        else
            echo "Simulating server response: slot=$SLOT (no path)"
            ros2 topic pub --once /mission/test_cmd std_msgs/String "data: 'VERIFY $SLOT'"
        fi
        ;;
    full)
        if [ -z "$2" ] || [ -z "$3" ]; then
            echo "Usage: $0 full <번호판> <슬롯> [전체경로]"
            echo "Example: $0 full 12가3456 17 0,1,5,1,0"
            exit 1
        fi
        PLATE=$2
        SLOT=$3
    ...
    exit)
        if [ -z "$2" ]; then
            echo "Usage: $0 exit <전체경로> <슬롯>  OR  $0 exit <슬롯>"
            echo "Example: $0 exit 0,1,5,1,0 17"
            echo "Example: $0 exit 17"
            exit 1
        fi
        if [ -n "$3" ]; then
            echo "Exit mission: full_path=$2 -> slot $3"
